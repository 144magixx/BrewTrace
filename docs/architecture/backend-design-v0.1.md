...
- `EXPLICIT_WORKFLOW`：默认模式，Planner 输出结构化步骤，Orchestrator 执行。
- `MODEL_TOOL_CALLING`：实验模式，让模型在受控工具集合内选择工具。

多 Agent 采用应用内逻辑角色：

- `InterviewAgent`：追问和信息补齐。
- `FlavorAgent`：风味联想。
- `MemoryAgent`：记忆召回和相似记录解释。
- `DraftAgent`：生成克制版、夸张版、锐评版文案。
- `ReviewAgent`：事实边界、风险和重复表达检查。
- `PublishingAgent`：发布包和发布流程推进。
...
Prompt 需要版本化，模型调用轨迹记录 prompt 版本、输入摘要和原始快照。
...
- 后端自动化验证使用 Java 21 应用内核、Fake Adapter 和离线行为测试 runner；前端自动化验证使用 Node 脚本检查核心工作台组件和流程约束。
...
- 将当前离线 Controller 形状替换为真实 REST Controller 注解和 HTTP 测试。
