...
- Maven 可通过本地/公司镜像完成无外部业务依赖的 Java 21 编译。
- 当前自动化测试覆盖了 REST envelope、事实边界、风味候选、Outbox、记忆召回、轨迹脱敏、发布确认和主动生图约束。
- 前端当前是离线可测试的 TypeScript 组件模型，还没有接入真实 React/Vite 运行时依赖。

### Codex 判断

- 对学习型 MVP 来说，先让领域模型、工具安全、记忆、轨迹和发布确认跑通，比一开始卡在依赖解析更有价值。
- 现有实现适合作为“可讲清楚的业务内核”，下一步学习重点应是把内核接入 Spring Boot、真实数据库和真实前端运行时。

### 待用户确认假设

- 是否接受先以离线内核验证业务链路，再单独安排一次“Spring Boot 真实运行时接入”切片。
- 真实小红书发布是否只做人工验证，不纳入默认 CI 或本地自动测试。
